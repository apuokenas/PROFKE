...
public class AverageOfThreeNumbersTest {
    ...
    private void correctOutput(String first, String second, String third) {
        ...
        String[] lines = new String[] { "Give the first number:", "Give the second number:", "Give the third number:",
                "The average is "
                        + (1.0 * Integer.valueOf(first) + Integer.valueOf(second) + Integer.valueOf(third)) / 3 };

        List<String> rows = rows(io.getSysOut().trim());

        assertEquals("The output was expected to contain " + lines.length + " line" + ((lines.length == 1) ? "" : "s")
