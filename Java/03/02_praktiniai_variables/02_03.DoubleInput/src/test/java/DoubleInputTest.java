...
public class DoubleInputTest {
    ...
    private void correctOutput(String number) {
        ...
        String[] lines = new String[] { "Give a number:", "You gave the number " + number };

        List<String> rows = rows(io.getSysOut().trim());

        assertEquals("Output was expected to contain " + lines.length + " row" + ((lines.length == 1) ? "" : "s")
                + ". Now there were " + rows.size() + " rows.", lines.length, rows.size());
        for (int i = 0; i < rows.size(); i++) {
            assertEquals(
                    "Row number " + (i + 1) + " produces incorrect output when the given number is " + number
                            + ".\nExpected string:\n" + lines[i] + "\nBut output was:\n" + rows.get(i),
